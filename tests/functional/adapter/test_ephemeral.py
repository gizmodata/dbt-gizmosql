...
class TestEphemeralErrorHandlingGizmoSQL(BaseEphemeralErrorHandling):
    pass


class TestEphemeralMultiGizmoSQL(BaseEphemeralMulti):
    def test_ephemeral_multi(self, project):
        ...
        # GizmoSQL uses "dbt" as the database name (from DATABASE_FILENAME=dbt.db)
        expected_sql = (
            ...
            " where gender = 'Male'"
            "union all"
            "select * from __dbt__cte__base_copy"
            " where gender = 'Female');"
        )
